...
function rhd_enqueue_scripts()
{
	...
	wp_register_script( 'rhd-ajax', RHD_THEME_DIR . '/js/ajax.js', array( 'jquery', 'packery' ), null, true );
	...
	if ( RHD_AJAX_PAGINATION && ( is_page( 'blog' ) || is_archive() || is_home() || is_search() ) ) {
...
function rhd_load_more( WP_Query $q = null )
{
	...
	echo '<nav class="pagination pagination-load-more" data-current-page="' . $paged . '">';
...
/**
 * rhdSearchFilter function.
 *
 * @access public
 * @param mixed $query
 * @return void
 */
function rhdSearchFilter( $query )
{
	if ( $query->is_search) {
