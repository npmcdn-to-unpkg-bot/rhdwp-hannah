...
function rhd_enqueue_scripts()
{
	...
	wp_register_script( 'rhd-plugins', RHD_THEME_DIR . '/js/plugins.js', array( 'jquery' ), null, false );
	wp_register_script( 'imagesloaded', RHD_THEME_DIR . '/js/vendor/imagesloaded/imagesloaded.pkgd.min.js', array( 'jquery' ), '3.2.0', false );
...
/**
 * rhd_check_app_quota_day function.
 *
 * @access public
 * @param mixed $reply_array
 * @return void
...
function rhd_check_app_quota_day( $reply_array )
{
	...
	$day_sql = $wpdb->prepare( "SELECT COUNT(*) FROM " . $appointments->app_table . " WHERE user=%d AND (status='pending' OR status='confirmed' OR status='paid' ) and DATEDIFF(start, %s)=0", array(
		$current_user->ID,
		$day
	 ) );
